  ...
  const {
    ...
    queryFn: async () => {
      ...
      const res = await fetch(
        `${API_BASE}/api/audit?auditId=${selectedAuditId}`,
        {
        ...
        {isLoading ? (
          ...
          <div className="flex flex-1 overflow-hidden">
            ...
            <div className="w-1/2 p-6 overflow-y-auto space-y-5">
              ...
              {/* Policy reference — now uses matched_rule + policy_notes */}
              <section>
                <h4 className="text-[10px] font-bold uppercase opacity-40 mb-2">
                  Policy Reference
                </h4>
        ...
        <div className="grid gap-4">
          {filtered?.map((report: any) => {
            ...
            return (
              <div
                key={report.id}
                className="bg-base-100 border border-base-300 rounded-xl hover:border-primary transition-all overflow-hidden"
              >
                {/* Summary row */}
                <button
